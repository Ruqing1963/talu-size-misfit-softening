...
with the dislocation strain field from the anisotropic Stroh sextic formalism using Ta elastic constants C₁₁/C₁₂/C₄₄ = 266.0/158.2/87.4 GPa (Zener anisotropy A = 1.622, K = 194.1 GPa).
...
Line tensions use the Dewit--Koehler orientation dependence Γ = E + d²E/dθ²:
...
For Ta, 1−2ν = 0.32: an edge dislocation is only 0.53× as stiff as the common μb²/2 estimate, while a screw is 4.19× stiffer than an edge. Since Friedel statistics give \(\tau \propto \Gamma^{-1/2}\), the naive estimate is wrong by 37 \% for edges.
...
Solute--solute interaction enters through the interchange energy ω = 2ε\_AB − ε\_AA − ε\_BB, giving the lattice-gas Hamiltonian
...
Warren--Cowley analysis of the two-dimensional atmosphere gives a result that matters for experimental interpretation. In the saturated core, c\_Lu → 1 and
...
\textbf{Set B --- screw core binding (13 cells).} 7 × 7 × 1 quadrupolar dipole cell, 147 atoms, anisotropic Stroh displacement field, periodicity restored by the measured slip offset. Lu is placed at ten symmetry-inequivalent columns within 7.5 Å plus a far reference at 28.1 Å. Angular sampling is deliberate: the pairs (4.08 Å, 19.1°)/(4.09 Å, 100.9°) and (5.54 Å, 46.4°)/(5.56 Å, 73.8°) sit at equal radius and different azimuth and measure the cos 3θ amplitude directly. Defines \(U_{\rm cap}^{\rm DFT} = \max_{\rm site}|E_b|\).

\textbf{Set C --- nudged elastic band (14 images).} Two runs of seven images. \textbf{Both cores translate by the same glide vector} \(g = 2.6995\) \AA~(\(= a_0\sqrt{6}/3\), easy core to next easy core, not to the intervening hard core at \(a_0\sqrt{6}/9\)): the dipole separation and hence the elastic interaction energy are unchanged, and the net plastic strain is \((b-b)\Delta x/A = 0\), so all images share one cell and no spurious elastic work enters the barrier. Images are built by regenerating the Stroh field at intermediate core positions, not by linear interpolation of coordinates, which passes through configurations with atoms displaced off their ⟨111⟩ columns.
...
Bibliographic data are given in \texttt{paper/references.bib} in the repository. All
eighteen entries have been checked against publisher records.

Nine errors were corrected in the process, and we note them because they are
the kind that survive casual review. Two were wrong titles: the Varvenne
\emph{et al.} 2016 article, and the Guggenheim 1952 book subtitle. Two were wrong
locators: Labusch 1970 is \emph{phys. stat. sol.} \textbf{(b)}, not (a), and de Wit \&
Koehler 1959 appeared on 1 December 1959, hence issue 5. One was a wrong page
range, Featherston \& Neighbours 1963 running to 1333. Three were wrong DOIs ---
Christian 1983 is \texttt{10.1007/BF02664806}, Fleischer 1962 ends in check digit 6
rather than X, and Stroh 1958 is \texttt{10.1080/14786435808565804}. One was a wrong
ISBN check digit for Hirth \& Lothe 1982. Kocks, Argon \& Ashby 1975 was
additionally mis-typed as a journal article; it is a monograph filling
\emph{Progress in Materials Science} vol.~19.

A wrong DOI is not a cosmetic defect: preprint servers resolve DOIs
automatically, so it creates a permanent link to the wrong paper. Two
independently supplied bibliographies, each described as complete or fully
verified, contained these errors between them, and one was also present in an
earlier draft of our own file.

Two entries carry a caveat rather than a correction. Friedel 1964 and Warren
1969 both predate general ISBN use, and the ISBNs commonly attached to them
belong to later reprints; readers should cite the edition they consulted.
Dezerald \emph{et al.} 2014 has an erratum, Phys. Rev.~B \textbf{91}, 019902 (2015).
