...
paper/    manuscript.md / manuscript_zh.md   source of truth, edit these
          references.bib                     all 18 entries verified
          main.tex + preamble.tex + body.tex  modular build
          main_standalone.tex                 single self-contained file,
                                              for arXiv or journal submission
          main.pdf / main_zh.pdf              16 pages each
          build.sh / build_zh.sh              markdown -> LaTeX -> PDF

The manuscripts are written in Markdown and converted by pandoc; `main.tex` is
only a four-line wrapper that pulls in `preamble.tex` (formatting) and
`body.tex` (the 447-line converted text). Editing `body.tex` directly will be
overwritten on the next build — edit `manuscript.md` instead. Each build also
emits `main_standalone.tex`, with preamble and body merged into one file for
submission systems that expect a single source.
...
**References: 18 of 18 verified**, all cited in the text and printed by BibTeX
(`unsrtnat`, numeric). The per-entry audit trail lives in a non-standard
`verification` field, which BibTeX ignores, so the notes stay in the source
record without reaching the typeset page. Verification caught nine errors, documented
